...
function InputBar({ value, onChange, onSend, onKeyDown, disabled, isMobile, hasSidebar, inputRef, mode, onCycleMode }) {
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                {/* ── DESKTOP: Mode pill row above the input ── */}
                {!isMobile && (
                    <div style={{
                        display: "flex",
                        justifyContent: "center",
                        marginBottom: 10,
                        marginLeft: hasSidebar ? 220 : 0,
                        paddingLeft: 40,
                        paddingRight: 40,
                    }}>
                        <div style={{
                            ...
                            {["archive", "web", "curator"].map(m => (
                                <button
                                    key={m}
                                    onClick={() => onCycleMode(m)}
                                    disabled={disabled}
                                    style={{
                                        background: mode === m ? GOLD : "transparent",
                                        border: "none",
                                        borderRadius: 16,
                                        color: mode === m ? "#0f0f0f" : MUTED,
                                        fontFamily: "inherit",
                                        fontSize: 11,
                                        fontWeight: 600,
                                        padding: "6px 16px",
                                        ...
                                        transition: "all 0.2s",
                                        whiteSpace: "nowrap",
                                        boxShadow: mode === m && m === "curator" ? `0 0 10px ${GOLD}44` : "none",
                                    }}
                                >
                                    {m === "archive" ? "Archive" : m === "web" ? "Web" : "Curator ✦"}
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <div style={{ width: "100%", maxWidth: 700, display: "flex", gap: 10, alignItems: "center" }}>
                    ...
                    {isMobile && (
                        <button
                            onClick={() => onCycleMode()}
                            disabled={disabled}
                            title="Tap to cycle: Archive → Web → Curator"
                            style={{
                                background: mode !== "archive" ? GOLD : "transparent",
                                border: `1.5px solid ${GOLD}`,
                                borderRadius: 12,
                                color: mode !== "archive" ? "#0f0f0f" : GOLD,
                                fontFamily: "inherit",
                                fontSize: 11,
                                fontWeight: 600,
                                padding: "14px 10px",
                                ...
                                transition: "all 0.2s",
                                minWidth: 56,
                                boxShadow: mode === "curator" ? `0 0 10px ${GOLD}55` : "none",
                            }}
                            onMouseEnter={e => { if (!disabled) e.currentTarget.style.opacity = "0.8" }}
                            onMouseLeave={e => { e.currentTarget.style.opacity = "1" }}
                        >
                            {mode === "archive" ? "ARC" : mode === "web" ? "WEB" : "CUR ✦"}
                        </button>
                    )}
                    <input
                        ref={inputRef}
                        ...
                        onChange={onChange}
                        onKeyDown={onKeyDown}
                        placeholder={listening ? "Listening..." : mode === "curator" ? "Ask The Curator..." : "Ask Jabril..."}
                        disabled={disabled}
                        style={{
...
function MainApp({ user, onSignOut, onAuthNeeded, showInstall = false }) {
    ...
    const cycleMode = (direct) => direct ? setMode(direct) : setMode(m => m === "archive" ? "web" : m === "web" ? "curator" : "archive")
    ...
    async function send(queryOverride) {
        ...
        const loadingMsg = { id: `l-${Date.now()}`, role: "loading", text: mode === "curator" ? "The Curator is thinking..." : mode === "web" ? "Searching the web..." : "Consulting the Archive..." }
        ...
        const webhookQuery = mode === "web" ? `web: ${query}` : query

        // Build webhook body — curator mode passes flag + prompt for n8n to use
        const webhookBody = mode === "curator"
