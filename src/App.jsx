                                ...
                                {msg.role === "ai" && !isMobile && (
                                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                                        ...
                                        {index > 0 && messages[index - 1]?.role === "user" && (
                                            <button
                                                onClick={() => send(`deepdive: ${messages[index - 1].text.replace(/^(deepdive:\s*)+/i, "")}`)}
                                                disabled={loading}
                                                style={{ ...actionBtnStyle(false), opacity: loading ? 0.6 : 1 }}
                                ...
                                <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 16, marginBottom: 8, flexWrap: "wrap" }}>
                                    ...
                                    {index > 0 && messages[index - 1]?.role === "user" && (
                                        <button
                                            onClick={() => send(`deepdive: ${messages[index - 1].text.replace(/^(deepdive:\s*)+/i, "")}`)}
                                            disabled={loading}
                                            style={{ ...mobileActionBtnStyle(false), opacity: loading ? 0.6 : 1 }}
                                        >
                                            Dive Deeper
                                        </button>
                                    )}
